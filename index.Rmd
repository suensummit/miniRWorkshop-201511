---
title: "動手玩資料：R 語言實戰營 - Data PlayeR Workshop"
author: "Summit Suen"
date: "Nov 22, 2015"
output: html_document
---
> [勘誤 Errata](errata.html)
---

### 內容大綱
- 從小統計軟體到大數據語言
- 資料分析師的好朋友
- 把玩資料，還是被資料玩弄

---

## 從小統計軟體到大數據語言

```{r}
?'='
?'<-'
?'<<-'
?solve
?help
??help
```

```{r, warning=FALSE}
1 + 1
1:10
x <- c(1:10)
y <- c(2:5)
x
y
x + y
x * y
```

## [超展開數學教室：賴以威](http://opinion.udn.com/opinion/story/8159/1317267)

```{r}
temp <- proc.time()	## current time record
p <- 0
for (t in 0:100000){
  p <- p + 1/2*(1/4)^t
}
(spttime <- proc.time() - temp)
p
```

## R 語言的強項：向量運算

```{r}
# temp <- proc.time()	## current time record
# p <- 1/2
# t <- c(0:1000)
# p <- p*sum((1/4)^t)
temp <- proc.time()	## current time record
p <- 1/2*sum((1/4)^c(0:100000)) # calculate in one line
(spttime <- proc.time() - temp)
p
```

## [Practice] R Basic: 環境安裝與基本設定

```{r}
# live coding
```

## 資料分析師的好朋友

## [Practice] R Data: Play Analysis with R

```{r}
# live coding
```

## 把玩資料，還是被資料玩弄

## [Practice] R Data: Beyond Analysis

## ETL

```{r}
# install.packages("Lahman")
library(Lahman)
library(dplyr)
totalRS <- Teams %>% select(yearID, R, G) %>% mutate(AvgRperG = R/G) %>% group_by(yearID) %>% summarise(sum(AvgRperG))
names(totalRS) <- c("yearID", "RUN")
head(totalRS)
```

## EDA: Visualization

```{r, fig.width=14, fig.height=5}
library(ggplot2)
ggplot(data = totalRS, aes(x = yearID, y = RUN)) + stat_smooth() + geom_line()
```

```{r}
library(Lahman)
library(dplyr)
head(filter(Pitching, playerID == "wangch01"))
```

## 更多學習資源

- [Try R](http://tryr.codeschool.com/)
- [Taiwan R User Group 資料分析上手課程](http://taiwanrusergroup.github.io/DSC2014Tutorial/)
- [R 語言資料分析上手課程 ](https://github.com/TaiwanRUserGroup/DSC2015Tutorial)
- [木刻思課程平台](http://data-sci.info/)
- [R Basic with Quantmod 課程（Powered by 木刻思）](https://eduweb.sfi.org.tw/Course.aspx?id=7786)